...
allNftList = [
"0x7facb4502da6e0021fad92f34b7e75fb695a63cf"
] 

allNftAmounts = ["121"] 

def updateSmudgeCatworldPrices():
    ...
    url = "https://raregems.io/energyweb/smudge-cat-world"
...
#    percentage60day = (greenseapriceslistusd[0]/greenseapriceslistusd60day[0]*100)-100
    # -------------------------------------------------------------------------- #
    image = f"/images/SmudgeCatworld.webp"
    imageanimated = f"/animatedimages/SmudgeCatworld.webp"
    assettype = "ERC-721"
    if combinedpriceslistoriginal[3] == 'Raregems':
        cheapestpricemarketlink = f"https://raregems.io/energyweb/smudge-cat-world/{combinedpriceslistoriginal[2]}"
    ...
    db.child(f"{25}").update({"rank": carbonswapNftData["rank"]})
    db.child(f"{25}").update({"id": carbonswapNftData["id"]})
    db.child(f"{25}").update({"name": carbonswapNftData["name"]})
    db.child(f"{25}").update({"description": carbonswapNftData["description"]})
    db.child(f"{25}").update({"image": carbonswapNftData["image"]})
    db.child(f"{25}").update({"imageanimated": carbonswapNftData["imageanimated"]})
    db.child(f"{25}").update({"cheapestpriceoriginal": carbonswapNftData["cheapestpriceoriginal"]})
    db.child(f"{25}").update({"cheapestpricecurrency": carbonswapNftData["cheapestpricecurrency"]})
    db.child(f"{25}").update({"cheapestmarket": carbonswapNftData["cheapestpricemarket"]})
    db.child(f"{25}").update({"cheapestmarketlink": carbonswapNftData["cheapestpricemarketlink"]})
    db.child(f"{25}").update({"marketcap": carbonswapNftData["marketcap"]})
    db.child(f"{25}").update({"floorpricesevenday": carbonswapNftData["floorpricesevenday"]})
    db.child(f"{25}").update({"floorpricefourteenday": carbonswapNftData["floorpricefourteenday"]})
    db.child(f"{25}").update({"floorpricethirtyday": carbonswapNftData["floorpricethirtyday"]})
    db.child(f"{25}").update({"floorpricesixtyday": carbonswapNftData["floorpricesixtyday"]})
    db.child(f"{25}").update({"percentage7daycolor": carbonswapNftData["percentage7daycolor"]})
    db.child(f"{25}").update({"percentage14daycolor": carbonswapNftData["percentage14daycolor"]})
    db.child(f"{25}").update({"percentage30daycolor": carbonswapNftData["percentage30daycolor"]})
    db.child(f"{25}").update({"percentage60daycolor": carbonswapNftData["percentage60daycolor"]})
    db.child(f"{25}").update({"circulating": carbonswapNftData["circulating"]})
    db.child(f"{25}").update({"floorprice": carbonswapNftData["floorprice"]})
    db.child(f"{25}").update({"owners": carbonswapNftData["owners"]})
    db.child(f"{25}").update({"assettype": carbonswapNftData["assettype"]})
    print(f"Updated NFT with ID: {25}")