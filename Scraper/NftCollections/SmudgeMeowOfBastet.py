...
nftnameList = [
"Bastet","Kitty mummy","Kheper","Canopic","Scepter of Smudge","Poolparty","Ra"
]

allNftList = [
"0x4aa7e34d417afde3195bffae96d221fea0369241-1", "0x4aa7e34d417afde3195bffae96d221fea0369241-2", "0x4aa7e34d417afde3195bffae96d221fea0369241-3",
"0x4aa7e34d417afde3195bffae96d221fea0369241-4", "0x4aa7e34d417afde3195bffae96d221fea0369241-5", "0x4aa7e34d417afde3195bffae96d221fea0369241-6",
"0x4aa7e34d417afde3195bffae96d221fea0369241-7"
] 

descriptions = [
    "Bastet is the Genesis NFT",
    "The best friend of Bastet",
    "Scarab-faced god",
    "Mummification jar that comes in handy in afterlife",
    "The only way to tamper chaos",
    "Party hard but don't get wet",
    "God of the sun that loves to experiment"
]
...
def updateSmudgeMeowOfBastetPrices():
    ...
    # ------------------------------- Beatifulsoup ---------------------------------- #
        ...
        holders = 0
        url = "https://raregems.io/energyweb/smudge-meow-of-bastet/"+f"{i+1}"
    ...
    # ----------------------------------- 7 day ----------------------------------- #
        ...
        # -------------------------------------------------------------------------- #
        image = f"/images/Smudge_{i+1}.png"
        imageanimated = f"/animatedimages/Smudge_{i+1}.png"
        assettype = "ERC-1155"
        ...
        db.child(f"{i+11}").update({"rank": carbonswapNftData["rank"]})
        db.child(f"{i+11}").update({"id": carbonswapNftData["id"]})
        db.child(f"{i+11}").update({"name": carbonswapNftData["name"]})
        db.child(f"{i+11}").update({"description": carbonswapNftData["description"]})
        db.child(f"{i+11}").update({"image": carbonswapNftData["image"]})
        db.child(f"{i+11}").update({"imageanimated": carbonswapNftData["imageanimated"]})
        db.child(f"{i+11}").update({"cheapestpriceoriginal": carbonswapNftData["cheapestpriceoriginal"]})
        db.child(f"{i+11}").update({"cheapestpricecurrency": carbonswapNftData["cheapestpricecurrency"]})
        db.child(f"{i+11}").update({"cheapestmarket": carbonswapNftData["cheapestpricemarket"]})
        db.child(f"{i+11}").update({"cheapestmarketlink": carbonswapNftData["cheapestpricemarketlink"]})
        db.child(f"{i+11}").update({"marketcap": carbonswapNftData["marketcap"]})
        db.child(f"{i+11}").update({"floorpricesevenday": carbonswapNftData["floorpricesevenday"]})
        db.child(f"{i+11}").update({"floorpricefourteenday": carbonswapNftData["floorpricefourteenday"]})
        db.child(f"{i+11}").update({"floorpricethirtyday": carbonswapNftData["floorpricethirtyday"]})
        db.child(f"{i+11}").update({"floorpricesixtyday": carbonswapNftData["floorpricesixtyday"]})
        db.child(f"{i+11}").update({"percentage7daycolor": carbonswapNftData["percentage7daycolor"]})
        db.child(f"{i+11}").update({"percentage14daycolor": carbonswapNftData["percentage14daycolor"]})
        db.child(f"{i+11}").update({"percentage30daycolor": carbonswapNftData["percentage30daycolor"]})
        db.child(f"{i+11}").update({"percentage60daycolor": carbonswapNftData["percentage60daycolor"]})
        db.child(f"{i+11}").update({"circulating": carbonswapNftData["circulating"]})
        db.child(f"{i+11}").update({"floorprice": carbonswapNftData["floorprice"]})
        db.child(f"{i+11}").update({"owners": carbonswapNftData["owners"]})
        db.child(f"{i+11}").update({"assettype": carbonswapNftData["assettype"]})
